...
export default function LeftPanel({ site, activeSection }) {
  return (
    <aside className="left-panel">
      ...
      <nav className="left-nav" aria-label="On-page sections">
        {NAV.map((item) => (
          <a
            key={item.id}
            href={`#${item.id}`}
            className={`left-nav__item${activeSection === item.id ? ' active' : ''}`}
          >
            <span className="left-nav__line" aria-hidden="true" />
            {item.label}
          </a>
        ))}
      </nav>
      ...
      <div className="social-links">
        ...
        <a href={site.links.email} className="social-link" aria-label="Email">
          <EmailIcon />
        </a>
      </div>
    </aside>
